...
const Employees = () => {
  ...
  const fetchEmployees = async (token) => {
    try {
      ...
      setEmployees(response.data.data || []); // Ma'lumotni yangilash
    } catch (error) {
      console.error("Hodimlarni olishda xato:", error.response || error.message);
      toast.error("Hodimlarni olishda xatolik yuz berdi!");
      if (error.response?.status === 401) {
        const newToken = await refreshAuthToken();
        if (newToken) fetchEmployees(newToken);
      }
  ...
  const deleteEmployee = async (id) => {
    const isConfirmed = window.confirm("Haqiqatan ham ushbu hodimni o'chirmoqchimisiz?");
    if (!isConfirmed) return;
    ...
    try {
      ...
      console.error("Hodimni o'chirishda xato:", error.response || error.message);
      toast.error("Hodimni o'chirishda xatolik yuz berdi!");
      if (error.response?.status === 401) {
        const newToken = await refreshAuthToken();
        if (newToken) deleteEmployee(id); // Yangi token bilan qayta urinish
      }
    }
  };
